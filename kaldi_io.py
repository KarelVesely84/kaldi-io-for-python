...
def popen(cmd, mode="rb"):
    ...
    if mode == "r":
        ...
        threading.Thread(
            target=cleanup, args=(proc, cmd)).start()  # clean-up thread,
...
def read_vec_int(file_or_fd):
  ...
  """
    ...
    if binary == '\0B':  # binary flag
        ...
        # int-size
        vec_size = np.frombuffer(
            fd.read(4), dtype='int32', count=1)[0]  # vector dim
...
def write_vec_int(file_or_fd, v, key=''):
  ...
  """
    ...
    try:
        if key != '':
            fd.write(
                (key + ' ').encode())  # ark-files have keys (utterance-id),
...
def _read_compressed_mat(fd, format):
  ...
  """
    ...
    global_header = np.dtype(
        [('minvalue', 'float32'), ('range', 'float32'), ('num_rows', 'int32'),
         ('num_cols', 'int32')])  # member '.format' is not written,
    per_col_header = np.dtype(
        [('percentile_0', 'uint16'), ('percentile_25', 'uint16'),
...
def read_post(file_or_fd):
  ...
  """
    ...
    assert (fd.read(1).decode() == '\4')
    # int-size
    outer_vec_size = np.frombuffer(
        fd.read(4), dtype='int32', count=1)[0]  # number of frames (or bins)
...
def read_segments_as_bool_vec(segments_file):
  ...
  """
    ...
    assert (len(np.unique(
        [rec[1] for rec in segs])) == 1)  # segments with only 1 wav-file,
